...
|---|---|
| **[Generated Formal Theorem Proofs](https://github.com/AmitAminov/tactic-priors)** | A goal-blind 16,850-parameter tactic-frequency table proves **26.2%** of miniF2F-test vs **49.6%** for a ~7B neural prover at the *same* search budget — a floor provers are rarely measured against. |
...
| **[Agentic Research Advisor](https://github.com/AmitAminov/agentic-research-advisor)** | An autonomous paper-reproduction harness graded on the artifacts each run leaves on disk, not the agent's self-report — honest negatives are first-class. |
| **[Technical Interviewer](https://github.com/AmitAminov/technical-interviewer)** | A voice-driven, **offline-first** mock interviewer: every dependency degrades to a deterministic local fallback, and fetched web content is treated as untrusted (prompt-injection-hardened). |
